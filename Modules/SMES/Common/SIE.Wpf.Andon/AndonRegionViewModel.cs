...
namespace SIE.Wpf.Andon
{
    ...
    [Label("安灯区域管理")]
    public class AndonRegionViewModel : KZDataCollectionViewModel
    {
        /// <summary>
        /// 安灯管理构造函数
        /// </summary>
        public AndonRegionViewModel()
        {
            //InitRegionWorkstation
            InitWorkstation();
        }
        ...
        protected virtual void SetRefreshTimer()
        {
            ...
            var config = ConfigService.GetConfig(new RefreshAndonManageConfig(), typeof(AndonRegionViewModel));
        ...
        public void ShowAndonTriggerDialog(object andonId)
        {
            ...
            ui.MainView.ChildrenViews.ForEach(p=> { p.IsVisible = isAskMaterial; });    //凯中无叫料业务,隐藏子视图
            CRT.Workbench.ShowDialog(ui, w =>
            {
                w.Title = "安灯触发".L10N();
