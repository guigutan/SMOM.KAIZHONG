...
namespace SIE.Web.Andon.AndonStatisticsReports.Commands
{
    /// <summary>
    /// 导入导出共用方法
    /// </summary>
    public class AndonExcelHelper
    {
        #region Excel按路径导出
        /// <summary>
        /// 设置单元格样式
        ...
        public MemoryStream ExportData(JObject dataView, List<Bitmap> images)
        {
            ...
            const string dataViewSheetName = "安灯统计报表";
            ...
            //ISheet sheet1 = workbook.CreateSheet("安灯统计报表");  //创建工作表 
            workbook.SetSheetOrder(dataViewSheetName, 0); //将抽样数据sheet放到最后
            ...
            if (sheetGridData != null)
            {
                try
                {
                    for (int i = 0; i < images.Count; i++)  //可能有多张控制图片
                    {
                        ...
                        {
                            byte[] bytes = null;
                            sheetGridData.PrintSetup.Landscape = true;
                            sheetGridData.DisplayGridlines = false;
                            var imgRowIndex = i * 20 + sheetGridData.LastRowNum + 3;
                            ...
                            IDrawing patriarch = sheetGridData.CreateDrawingPatriarch();  //创建绘制器
