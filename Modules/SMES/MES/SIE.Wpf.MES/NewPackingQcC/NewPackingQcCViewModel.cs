...
namespace SIE.Wpf.MES.NewPackingQcC
{
    ...
    [RootEntity]
    [Label("C包装采集")]
    public class NewPackingQcCViewModel : KZDataCollectionViewModel
    {
        public NewPackingQcCViewModel()
        {
            InitWorkstation();
        }
        #region 包装明细
        ...
        /// 包装明细
        /// </summary>
        public static readonly ListProperty<EntityList<NewPackingQcCModel>> PackageSnRecordListProperty = P<NewPackingQcCViewModel>.RegisterList(e => e.PackageSnRecordList, new ListPropertyMeta
        {
            HasManyType = HasManyType.Aggregation,
            DataProvider = e => (e as NewPackingQcCViewModel).LoadPackageSnRecordList()
        });

        /// <summary>
        /// 包装明细
        /// </summary>
        public EntityList<NewPackingQcCModel> PackageSnRecordList
        {
        ...
        /// </summary>
        [Label("已装箱总数量")]
        public static readonly Property<int> blueIntProperty = P<NewPackingQcCViewModel>.Register(e => e.blueInt);
        ...
        /// </summary>
        [Label("蓝标装箱总数")]
        public static readonly Property<int> blueZIntProperty = P<NewPackingQcCViewModel>.Register(e => e.blueZInt);
        ...
        /// </summary>
        [Label("蓝标")]
        public static readonly Property<string> XtBlueProperty = P<NewPackingQcCViewModel>.Register(e => e.XtBlue);
        ...
        /// </summary>
        [Label("原蓝标")]
        public static readonly Property<string> YXtBlueProperty = P<NewPackingQcCViewModel>.Register(e => e.YXtBlue);
        ...
        protected override void OnBarcodeChanged(ManagedPropertyChangedEventArgs e)
        {
                    ...
                    if (packingQc == null)
                    {
                        ...
                        if (BoxExChange == 1)
                        {
                            ...
                            else
                            {
                                ...
                                XtBlue = "";
                                YXtBlue = "";
                                Error = "换箱失败,新蓝标数量【" + xtBlue.PackageNum + "】,原蓝标数量【" + yXtBlue.PackageNum + "】,已装箱数据【" + detailSum + "】";
                                Tips = "请输入已装箱的蓝标!!!";
                                return;
                            }
                        }
            ...
            else
            {
                ...
                NewPackingQcCModel pqc = new NewPackingQcCModel();
                SIE.Core.Items.Item itemData = new SIE.Core.Items.Item();

                //批次标签等于null， 在去物料标签查询。
                itemLabel = RT.Service.Resolve<ItemLabelController>().GetPackingItemLabel(Barcode);
                if (itemLabel == null)
                {
                    Error = "当前物料标签【" + Barcode + "】不存在!";
                    Barcode = "";
                    return;
                }
                else
                {
                    ...
                    if (RT.Service.Resolve<WorkOrderBomController>().GetWorkOrderBom((double)WorkOrderId, itemLabel.ItemId))
                    {
                        Error = "当前物料标签【" + Barcode + "】物料号【" + itemLabel.Item.Code + "】,没有在工单【" + WorkOrder.No + "】的BOM里面!";
                        Barcode = "";
                        return;
                    }
                    ...
                    //获取产品
                    itemData = RT.Service.Resolve<WipBatchController>().GetItem(workOrderData.ProductId);
                    ...
                    //判断在批次标签中是否存在
                    wipBatch = RT.Service.Resolve<WipBatchController>().GetWipBatchPc(Barcode + "-BG");
                    if (wipBatch == null)
                    {
                        //当前工单批次总数
                        var batchQty = RT.Service.Resolve<WipBatchController>().GetWipBatchSumBatchQty((double)WorkOrderId);
                        //工单计划数
                        var planQty = RT.Service.Resolve<WipBatchController>().GetWoQty((double)WorkOrderId);
                        if ((batchQty + itemLabel.Qty) <= planQty)
                        {

                            //把物料标签放入到批次标签里面
                            if (!SaveWipBatch(itemLabel, (double)WorkOrderId, "包装"))
                            {
                                Error = "物料标签保存失败!";
                                Barcode = "";
                                return;
                    ...
                    else
                    {
                        if (wipBatch.Isuse)
                        {
                            Error = "批次标签【" + Barcode + "-BG】已经使用!";
                            Barcode = "";
                            return;
                        }
                        if (wipBatch.IsSuspectProduct == YesNo.Yes)
                        {
                            Error = "批次标签【" + Barcode + "-BG】是可疑品,不允许使用!";
                            Barcode = "";
                            return;
                        }
                    }
                ...
                RF.Save(blueBable);
                //保存包装数据
                var saveString = RT.Service.Resolve<DispatchController>().SavePackingQcC(WorkOrder, (double)KZWorkstation.ResourceId, KZWorkstation.Resource.Name, Barcode + "-BG", itemLabel, wipBatch, ref packingQc, packingDetail, 0);
                if (saveString != "")
                {
                ...
                if (blueInt == blueZInt)
                {
                    MessageBox.Show("蓝标的装箱数量：" + blueInt + ",已经装箱完成!", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    BoolBlue = true;
                    XtBlue = "";
                    blueInt = 0;
                    XtBlue = "";
                    ...
                    record.Report = ReportType.ItemLabel;
                    string submitMessage = RT.Service.Resolve<PackingQcController>().SubmitData(packingQc);
        ...
        /// <returns></returns>
        public bool SaveWipBatch(ItemLabel itemLabel, double workOrderId, string processCode = null)
        {
            WipBatch wipBatch = new WipBatch();
            wipBatch.BatchNo = itemLabel.Label + "-BG";
